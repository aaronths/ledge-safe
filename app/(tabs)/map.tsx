...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setSavedLocations(snap.docs.map((d) => ({ id: d.id, ...d.data() } as SavedLocation)));
    });
  ...
  const reverseGeocode = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (results.length > 0) {
        const r = results[0];
        const parts = [r.district ?? r.subregion, r.city ?? r.region, r.isoCountryCode].filter(Boolean);
  ...
  const handleRegionChangeComplete = useCallback<NonNullable<MapViewProps["onRegionChangeComplete"]>>(
    (newRegion) => {
      setIsMoving(false);
      setCenterCoordinates({ latitude: newRegion.latitude, longitude: newRegion.longitude });
  ...
  const handleToggleSave = useCallback(async () => {
    ...
    if (currentlySaved) {
      ...
      setIsSaving(true);
      try {
        await deleteDoc(doc(db, "users", user.uid, "savedLocations", currentlySaved.id));
      } catch {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        for (const r of results.slice(0, 4)) {
          ...
          if (rev.length > 0) {
            const place = rev[0];
            const parts = [place.name, place.city ?? place.region, place.isoCountryCode].filter(Boolean);
            named.push({ name: parts.join(", "), latitude: r.latitude, longitude: r.longitude });
          }
        }
          ...
          <View style={styles.searchResults}>
            {searchResults.map((result, i) => (
              ...
              >
                ...
                <Text style={styles.searchResultText} numberOfLines={1}>{result.name}</Text>
              </Pressable>
            ))}
          </View>
        )}
      ...
      <View style={styles.mapTypeContainer}>
        {MAP_TYPES.map((type) => (
          <Pressable
            ...
            style={[styles.mapTypeBtn, mapType === type.value && styles.mapTypeBtnActive]}
          >
            <Text style={[styles.mapTypeBtnText, mapType === type.value && styles.mapTypeBtnTextActive]}>
              {type.label}
            </Text>
          </Pressable>
        ))}
      ...
      <View style={styles.bottomPanel}>
        <View style={styles.locationRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.coordinates}>
              {centerCoordinates.latitude.toFixed(5)}, {centerCoordinates.longitude.toFixed(5)}
            </Text>
          </View>
        ...
        {savedCoordinates && (
          <View style={styles.savedBadge}>
            ...
            <Text style={styles.savedText}>Analysis saved · {savedCoordinates.name}</Text>
          </View>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  crosshair: { width: 32, height: 32, justifyContent: "center", alignItems: "center" },
  crosshairMoving: { transform: [{ translateY: -6 }] },
  crosshairLineVertical: {
    position: "absolute", width: 2, height: 32,
    backgroundColor: "#c9a84c", borderRadius: 1, opacity: 0.9,
  },
  crosshairLineHorizontal: {
    position: "absolute", width: 32, height: 2,
    backgroundColor: "#c9a84c", borderRadius: 1, opacity: 0.9,
  },
  crosshairDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#c9a84c" },
  crosshairShadow: {
    width: 12, height: 4, borderRadius: 6,
    backgroundColor: "rgba(0,0,0,0.2)", marginTop: 4,
  },
  ...
  searchBar: {
    ...
    backgroundColor: "#0f1f3d", borderRadius: 14,
    paddingHorizontal: 16, paddingVertical: 13, gap: 10,
    borderWidth: 1.5, borderColor: "#c9a84c40",
    shadowColor: "#000", shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.4, shadowRadius: 8, elevation: 8,
  },
  searchInput: { flex: 1, color: "#e2e8f0", fontSize: 15, padding: 0 },
  searchResults: {
    backgroundColor: "#0f1f3d", borderRadius: 14, marginTop: 6,
    borderWidth: 1.5, borderColor: "#c9a84c40", overflow: "hidden",
    shadowColor: "#000", shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.4, shadowRadius: 8, elevation: 8,
  },
  searchResultItem: {
  ...
  mapTypeContainer: {
    ...
    flexDirection: "column", gap: 4, zIndex: 9,
  },
  mapTypeBtn: {
  ...
  bottomPanel: {
    ...
    borderTopColor: "#1e3a5f", borderTopWidth: 1,
    paddingHorizontal: 20, paddingTop: 16, paddingBottom: 36, gap: 10,
  },
  locationRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  locationName: { color: "#e2e8f0", fontSize: 16, fontWeight: "600" },
  locationNameLoading: { color: "#64748b", fontSize: 14 },
  coordinates: { fontSize: 11, color: "#475569", marginTop: 2, fontVariant: ["tabular-nums"] },
  saveBtn: {
  ...
  button: { backgroundColor: "#c9a84c", borderRadius: 12, paddingVertical: 14, marginTop: 2 },
  buttonPressed: { opacity: 0.8 },
  buttonText: { color: "#0f1f3d", textAlign: "center", fontSize: 15, fontWeight: "700" },
  savedBadge: {
    backgroundColor: "#1e3a5f", borderRadius: 8, paddingVertical: 8,
